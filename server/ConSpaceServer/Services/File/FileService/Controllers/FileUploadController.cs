...
public class FileUploadController : ControllerBase
{
    ...
    public async Task<IActionResult> PostImage([FromForm] FileModel file, string userId, bool isAvatar = false)
    {
        ...
        var (originalFileName, originalFileExt) =
            FileFormatValidator.GetOriginalFileNameAndExtension(file.FormFile.FileName);
        ...
        var uploadedImg = new FileDTO(originalFileName, originalFileExt, fileSize, uniqueFileName, uniqueFilePath,
            userId, isAvatar);
    ...
    public async Task<IActionResult> PostVideo(string userId)
    {
        ...
        string originalFileExt = String.Empty;
            ...
            if (hasContentDispositionHeader)
            {
                ...
                else
                {
                    (originalFileName, originalFileExt) =
                        FileFormatValidator.GetOriginalFileNameAndExtension(contentDisposition.FileName.Value);
        ...
        var uploadedVideo = new FileDTO(originalFileName, originalFileExt, _fileSizeLimit, uniqueFileName,
            uniqueFilePath, userId);
    ...
    public async Task<IActionResult> PostMultipleImages([FromForm] MultipleFilesModel files, string userId)
    {
        ...
        foreach (var file in files.formFiles)
        {
            ...
            var (originalFileName, originalFileExt) =
                FileFormatValidator.GetOriginalFileNameAndExtension(file.FileName);
            var (uniqueFileName, uniqueFilePath) = FileFormatValidator.GetUniqueFileNameAndPath(_path);
            ...
            var uploadedImg = new FileDTO(originalFileName, originalFileExt, fileSize, uniqueFileName,
                uniqueFilePath, userId, false);
            _repo.UploadFile(uploadedImg);
